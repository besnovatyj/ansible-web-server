...
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `70-release-db`      | `70-release` + импорт `mysql_dump/dump.sql` — ⚠ РАЗРУШИТЕЛЬНО, перезаписывает БД                                                                           |
| `70-release`         | git clone/pull из GitHub + composer install + php init; **это же — каждая последующая выкладка**                                                           |
| `80-certbot-staging` | (Проверка) выпуск против staging CA — отладка DNS/пайплайна без расхода прод-квот (сертификат недоверенный)                                                |
| `80-certbot-force`   | (Принудительное обновление после проверки) принудительный боевой перевыпуск — разово ПОСЛЕ staging-теста                                                   |
| `80-certbot`         | (Обычный выпуск) сертификаты Let's Encrypt. Предусловие: DNS **всех** доменов (main, www, adm, files) указывает на сервер — pre-flight проверит каждый сам |
