...
`~/.ssh/<domain>/` (`local_ssh_keys_dir` = `{{ local_home_path }}/.ssh/{{ domain_name }}`,
напр. `~/.ssh/bes-v.ru/`; нативная ext4 — Unix-права/chattr работают, ключи изолированы
от личных в ~/.ssh, рядом с плейбуком больше не хранятся):
